...
# Tarefa 1 - Sumarizar os dados com as médias de cada coluna de dataset.
library(dplyr)
medias_iris <- summarize(group_by(dados_iris, Species),
                         media_sepal_lenth = mean(Sepal.Length),
                         media_sepal_width = mean(Sepal.Width),
                         media_pedal_length = mean(Petal.Length),
                         media_pedal_width = mean(Petal.Width))
View(medias_iris)
...
# para as 3 categorias de uma variável categórica.
library(ggplot2)
